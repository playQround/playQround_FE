...
const CreateRoomWindow = ({ViewCreateRoom}) => {
    ...
    const CreateRoom = (event) => {
        ...
        const data = {
            roomName, maxPeople, cutRating, roomPublic
        };
        ...
        axios.post('http://localhost:3000/api/rooms', data)
            .then(response => {
        ...
        <div className="CreateRoomMain">
            <div className="CreateRoomContent">
                ...
                <form className='CreateRoomBox' onSubmit={CreateRoom}>
                    <label> 방 이름
                    <input type='text' placeholder='방 이름' value={roomName} onChange={ChangeRoomName}/>
                    </label>
                    
                    <label> 최대 인원 </label>
                    <input type='number' placeholder='최대 인원' value={maxPeople} onChange={ChangeMaxPeople}/>
                    
                    
                    <label> 입장 점수 </label>
                    <input type='number' placeholder='최소 레이팅 점수' value={cutRating} onChange={ChangeCutRating}/>
                ...
                <button className='Cancel' onClick={ViewCreateRoom}> Cancel</button>
            </div>
        </div>
    )
}
